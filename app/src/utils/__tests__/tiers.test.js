...
describe('tierFor', () => {
  it('maps punya to tiers at exact boundaries (mirrors DB tier_for)', () => {
    expect(tierFor(0)).toBe('Jijnasu')
    expect(tierFor(99)).toBe('Jijnasu')
    expect(tierFor(100)).toBe('Sadhaka')
    expect(tierFor(399)).toBe('Sadhaka')
    expect(tierFor(400)).toBe('Tapasvi')
    expect(tierFor(999)).toBe('Tapasvi')
...
describe('tierProgress', () => {
  it('reports progress toward the next tier', () => {
    ...
    expect(tp.current).toBe('Tapasvi')
