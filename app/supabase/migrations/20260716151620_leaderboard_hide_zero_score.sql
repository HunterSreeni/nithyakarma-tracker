-- B4: get_leaderboard returned up to 50 rows including score-0 profiles, so a
-- sparse board could show inactive/never-active users. Filter those out,
-- except the caller's own row (always shown so they can see where they stand).
...
begin
  ...
  if p_scope = 'kids' then
    ...
    where score > 0 or is_me
    order by score desc, streak desc limit 50;
  else
    return query
    select * from (
