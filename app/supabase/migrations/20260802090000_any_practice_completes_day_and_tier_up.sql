-- Product decision (2026-08-02): day-completion required EVERY affects_streak
-- practice to be logged (bool_and). Reported as "temple visit doesn't update my
-- streak" - working as designed, but not what's wanted: marking any ONE
-- scheduled practice should keep the streak alive. Punya still only comes from
-- the practices you actually mark (unconditional, unaffected by this change);
-- this only changes what gates the streak/day_complete flag. bool_and -> bool_or.
--
-- Also surfaces tier-up as a first-class field (tier_up) instead of only the
-- silent freeze-cap top-up, so the client can show a tier-up celebration.
...
begin
  ...
  v_tier_up := tier_for(v_subject_punya) is distinct from tier_for(v_subject_punya - pr.punya_value);
  ...
  -- completes the day (bool_or) - not all of them (see header comment).
  select bool_or(
