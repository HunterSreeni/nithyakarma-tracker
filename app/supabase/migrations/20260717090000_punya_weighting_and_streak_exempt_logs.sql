-- Two fixes to karma/streak calculation:
--
-- 1. Punya was a flat +5 per log regardless of practice - a Sandhyavandhanam
--    slot, a 108-count japam, and a single Hanuman Chalisa verse mark all
--    earned the same reward. Replace with practices.punya_value (admin-tunable,
--    defaults to 5 so nothing changes unless explicitly reweighted below).
--
-- 2. LearningPage marks a verse learned by calling submit_practice_log for the
--    Hanuman Chalisa practice - the exact same RPC a genuine completion uses.
--    That let one verse (of 43) grant full punya AND drive both the per-practice
--    streak and the subject's overall daily streak, indefinitely, with no real
--    practice done. Add a per-log counts_toward_streak flag (not a per-practice
--    flag - genuinely reciting the full Chalisa from the Today page must still
--    build a streak) so a caller can opt a specific log out of streak effects
--    while still recording it and awarding punya.

alter table public.practices add column punya_value int not null default 5;

update public.practices set punya_value = 8
  where slug in ('vishnu-sahasranamam', 'lalitha-sahasranamam', 'soundarya-lahari',
                 'shiva-panchakshari-japam', 'narayaneeyam', 'bhagavad-gita', 'bhagavatam');
update public.practices set punya_value = 12
  where slug in ('sri-rudram', 'devi-mahatmyam');

alter table public.practice_logs add column counts_toward_streak boolean not null default true;

-- Adding p_award_streak as a new trailing param would create a second overload
-- (as happened with p_local_date - see 20260716144729) rather than replacing
-- the existing one. Drop the old arity first.
drop function if exists public.submit_practice_log(uuid, text, int, date);
...
begin
  ...
  -- Only logs that count toward streak can complete the day for streak purposes.
  select bool_and(
    case when p2.is_sandhyavandhanam then
      (select count(*) from practice_logs pl where pl.user_practice_id = up2.id and pl.log_date = v_today and pl.counts_toward_streak) >= 3
    else
...
revoke execute on function public.submit_practice_log(uuid, text, int, date, boolean) from anon, public;
grant execute on function public.submit_practice_log(uuid, text, int, date, boolean) to authenticated;
