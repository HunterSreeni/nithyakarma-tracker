-- A practice whose logs never count toward the streak must not be able to BLOCK
-- day completion either. The Learning page submits with p_award_streak = false, so
-- hanuman-chalisa logs write counts_toward_streak = false. Because that practice is
-- cadence 'daily', it joined the day-completion bool_and set every day and its
-- branch could never be satisfied from the Learning page - so once a user marked a
-- single verse, their day could never complete again and the overall streak froze.
--
-- Declare the intent on the practice instead of inferring it from log rows.
alter table public.practices
  add column if not exists affects_streak boolean not null default true;

comment on column public.practices.affects_streak is
  'When false, this practice is excluded from the day-completion bool_and in submit_practice_log. Learning-style practices earn punya but neither advance nor block the streak.';

update public.practices set affects_streak = false where slug = 'hanuman-chalisa';

-- Parameter defaults must be repeated verbatim; Postgres refuses to drop them via
-- CREATE OR REPLACE, and omitting them would break every call that relies on them.
