...
## 7. Phase 2 - future roadmap (NOT in current scope)

Ideas parked for after the current feature set ships to the Play Store. Captured
here so they are not lost; each needs its own design + test pass when picked up.

### 7.1 Thithi-based observances (lunar days)
...
### 7.2 Panchangam / calendar integration
